...
import '../feature_scanner/di/scanner_page_with_dependencies.dart';
import '../feature_splash/di/splash_page_with_dependencies.dart';
import '../feature_splash/services/splash_service.dart';
import '../feature_success/di/success_page_with_dependencies.dart';
...
part 'router.g.dart';
part 'routes/scanner_routes.dart';
...
class AppRouter {
  ...
  late final _GoRouterRefreshStream _refreshListener = _GoRouterRefreshStream(
    // A potential reason to refresh the navigation is if
    // the authentication on an app changes
  ...
  FutureOr<String?> _pageRedirections(
    ...
    if (!hasPermissions) {
      return const ScannerRoute().location;
    }
